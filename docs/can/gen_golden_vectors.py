...
Check:  python3 gen_golden_vectors.py --check
        exits non-zero, naming the file, if the on-disk artefacts do not match
        what this generator would emit from the current contract. The version is
        PARSED from the contract's own version line -- there is deliberately no
        version constant in this file any more, so the two cannot split (a real
        incident: the 2026-08-02f bump initially updated a duplicated constant
        here but missed the contract's version line). Scope: this closes the
        contract-vs-artefacts gap inside this repository; the SCBDriver vendored
        copy of the header is still an explicit, manual sync.
"""
...
def contract_version(contract_text):
    m = re.search(r"Contract version: \*\*(.+?)\*\*", contract_text)
    if not m:
        sys.exit("cannot find the 'Contract version: **...**' line in %s" % CONTRACT)
    return m.group(1)
...
def build(contract_sha, version):
    vectors = {
        "contract": CONTRACT,
        "contract_version": version,
        "contract_sha256": contract_sha,
        "note": (
            "CAN identifiers are deliberately absent from the vectors: they are assigned in "
            "the contract (data 0x214, health 0x215) and injected by each implementation, so "
            "nothing here depends on the numbers. Frames are hex, byte 0 leftmost. DLC is 8 "
            "for every frame."
        ),
        "constants": {
...
def main():
    ...
    version = contract_version(contract_bytes.decode("utf-8"))

    vectors = build(contract_sha, version)
    js_text = json.dumps(vectors, indent=2) + "\n"
    hh_text = emit_header(vectors)
    ...
    if check:
        stale = []
        for path, want in ((js, js_text), (hh, hh_text)):
            try:
        ...
        if stale:
            sys.exit("STALE: %s do(es) not match the contract (version %s, sha256 %s); "
                     "run gen_golden_vectors.py and update both repositories' pins"
                     % (", ".join(stale), version, contract_sha))
        print("check OK: contract %s sha256 %s matches the committed artefacts"
    ...
    print("contract %s sha256 %s" % (version, contract_sha))
    print("wrote %s" % js)
    print("wrote %s" % hh)
